...
class controller():
    ...
    def read(self, params):
        ...
        logging.log_info('Controller reading database...', os.path.abspath(__file__), sys._getframe().f_lineno)
        status = "200"
        if data_response_port is None:
            status = "404"
            data_response_port = None
            data_response_host = None
            logging.log_warning('Port/host not found', os.path.abspath(__file__), sys._getframe().f_lineno)
        ...
        logging.log_info(json.dumps(get_port_response), os.path.abspath(__file__), sys._getframe().f_lineno)
    ...
    def create(self, params):
        try:
            ...
            logging.log_info('Controller crating...', os.path.abspath(__file__), sys._getframe().f_lineno)
            ...
            logging.log_warning("bad request {}".format(emsg), os.path.abspath(__file__), sys._getframe().f_lineno)
        ...
        if data_response is False:
            ...
            data_response = None
            logging.log_warning('Port occupied or version/name already exists in db', os.path.abspath(__file__), sys._getframe().f_lineno)
        if data_response is "no":
            status = "500"
            statusMessage = "Database error, invalid operation"
            data_response = None
            logging.log_error('Database error, invalid operation', os.path.abspath(__file__), sys._getframe().f_lineno)
        ...
        logging.log_info(json.dumps(create_port_response), os.path.abspath(__file__), sys._getframe().f_lineno)
